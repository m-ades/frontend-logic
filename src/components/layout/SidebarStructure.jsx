...
const studentStructure = [
  { id: 0, label: "Dashboard", link: "/student/dashboard", icon: <DashboardIcon /> },
  { id: 1, type: "divider" },
  {
    id: 2,
    label: "Assignments",
  ...
  { id: 5, label: "Contact", link: "/student/contact", icon: <MessageIcon /> },
  { id: 6, label: "Settings", link: "/student/settings", icon: <SettingsIcon /> },
];
...
const instructorStructure = [
  ...
  { id: 6, label: "Contact", link: "/instructor/contact", icon: <MessageIcon /> },
  { id: 7, label: "Settings", link: "/instructor/profile", icon: <SettingsIcon /> },
];
...
const mapStructure = (structure, baseLink, activityLinkPrefix) => {
    ...
    children: (chapter.subchapters || []).map((subchapter) => ({
      ...
      children: (subchapter.activities || []).map((activity) => ({
        ...
        link: activity.worksheet
          ? `${activityLinkPrefix}/${activity.worksheet.id}`
          : baseLink,
      })),
    })),
  }));
...
export function useSidebarStructure() {
    ...
    const loadAssignments = async () => {
      try {
        const assignments = await fetchJson(
          `/api/courses/${API_CONFIG.courseId}/assignments`
        );
        if (!isMounted) return;
  ...
  return useMemo(() => {
    ...
    return structure.map((item) => {
      if (item.id === 2) {
        const baseLink = isInstructor ? "/instructor/assignments" : "/student/assignments";
        const activityPrefix = isInstructor ? "/instructor/assignments" : "/student/assignment";
        return { ...item, children: mapStructure(assignmentStructure, baseLink, activityPrefix) };
      }
      if (item.id === 3) {
        const baseLink = isInstructor ? "/instructor/practice" : "/student/practice";
        const activityPrefix = isInstructor ? "/instructor/practice" : "/student/assignment";
        return { ...item, children: mapStructure(practiceStructure, baseLink, activityPrefix) };
      }
      return item;
    });
