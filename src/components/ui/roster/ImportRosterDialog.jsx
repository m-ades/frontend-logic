    ...
    reader.onload = (e) => {
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          const studentErrors = [];
          if (!student.name) {
            studentErrors.push(`Row ${i + 1}: Missing name`);
          }
          if (!student.email) {
            studentErrors.push(`Row ${i + 1}: Missing email`);
          } else if (!isValidEmail(student.email)) {
            studentErrors.push(`Row ${i + 1}: Invalid email format`);
          }
          if (!student.password) {
        ...
        <Box sx={{ mb: 3 }}>
          <Alert severity="info" sx={{ mb: 2 }}>
            ...
            <Typography variant="body2" component="div">
              • First row must be headers: <strong>name,email,password</strong>
              <br />
              • Each subsequent row represents one student
              <br />
              • Email addresses must be valid format
              <br />• Passwords must be at least 6 characters
            </Typography>
          </Alert>
