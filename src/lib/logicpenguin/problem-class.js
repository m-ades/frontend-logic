...
export default class LogicPenguinProblem extends HTMLElement {
    ...
    getIndicatorStatus() {
        ...
        ind.points = -1;
        // Check if indicator exists and points is an object (not a DOM element)
        // Note: indicator.points is initially a DOM element, but may be set to an object later
        const pointsObj = this?.indicator?.points;
        if (pointsObj && 
            typeof pointsObj === 'object' && 
            !(pointsObj instanceof HTMLElement) &&
            pointsObj !== null &&
            "awarded" in pointsObj) {
            ind.points = pointsObj.awarded;
        }
    ...
    processAnswer() {
        ...
        if ("myanswer" in this && "myproblemtype" in this) {
            // localCheck is async but we don't await it - that's fine, it handles errors internally
            localCheck(this).catch(err => {
                console.error("Error in localCheck:", err);
                this.setIndicator({
                    savestatus: 'malfunction',
                    successstatus: 'malfunction',
                    points: -1,
