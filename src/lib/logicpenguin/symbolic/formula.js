    ...
    class Formula {
        ...
        static from = function(s) {
            const toparse = Formula.syntax.stripmatching(
                Formula.syntax.inputfix(s).trim()
            );
        ...
        get opspot() {
            ...
            for (let i=0; i<this.parsedstr.length; i++) {
                ...
                if (openBrackets[c]) {
                    ...
                    if (bracketStack.length == 0 || bracketStack.pop() != closeBrackets[c]) {
                        this.syntaxError("unbalanced parentheses (mismatched brackets)");
                    }
                    currdepth--;
                    finaldepth--;
                }
                ...
                if (startswithq) {
                    ...
                    } else if (m.search(Formula.syntax.symbols.EXISTS) >= 0) {
